...
async function metrics(request, context) {
...
async function getCosts(credential, subscriptionId) {
    try {
        const costClient = new arm_costmanagement_1.CostManagementClient(credential);
...
async function getResourceCounts(credential, subscriptionId, resourceGroup) {
    try {
        const graphClient = new arm_resourcegraph_1.ResourceGraphClient(credential);
        const query = `
            Resources
...
async function getStorageMetrics(credential, subscriptionId, resourceGroup) {
    try {
        ...
        for await (const account of storageClient.storageAccounts.listByResourceGroup(resourceGroup)) {
            ...
            try {
                ...
                if (usedBytes === 0) {
                    try {
                        const blobClient = new storage_blob_1.BlobServiceClient(`https://${account.name}.blob.core.windows.net`, credential);
                        ...
                        for await (const container of blobClient.listContainers()) {
                            ...
                            if (containerCount <= 5) { // Limit to first 5 containers for performance
                                ...
                                for await (const blob of containerClient.listBlobsFlat()) {
                                    ...
                                    blobCount++;
                                    if (blobCount > 100)
                                        break; // Limit blobs per container
                                }
                            }
                        }
                    }
...
async function getOpenAIUsage(context) {
    try {
        ...
        try {
            const blobClient = storage_blob_1.BlobServiceClient.fromConnectionString(process.env.AzureWebJobsStorage || '');
...
async function getProjectMetrics(credential, subscriptionId, resourceGroup, context) {
    try {
        const metrics = [];
        // Get Application Insights data
        const aiClient = new arm_appinsights_1.ApplicationInsightsManagementClient(credential, subscriptionId);
...
async function getCostBreakdown(credential, subscriptionId) {
    try {
        const costClient = new arm_costmanagement_1.CostManagementClient(credential);
        const scope = `/subscriptions/${subscriptionId}`;
        const now = new Date();
...
async function getResourceDetails(credential, subscriptionId, resourceGroup) {
    try {
        const graphClient = new arm_resourcegraph_1.ResourceGraphClient(credential);
